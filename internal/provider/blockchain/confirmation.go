...
func NewConfirmationTracker(c ChainClient, cfg ChainConfig, ptx repository.PendingTxRepository, orders repository.OrderRepository, p kafka.EventPublisher) *ConfirmationTracker {
...
func (t *ConfirmationTracker) processOne(ctx context.Context, p *domain.PendingTx, latest uint64) {
	...
	if err != nil || receipt == nil {
		if err := t.PendingTxs.UpdateConfirmations(ctx, p.TxHash, 0, domain.PendingTxStatusReorged); err != nil {
	...
	if receipt.Status != 1 {
		if err := t.PendingTxs.UpdateConfirmations(ctx, p.TxHash, 0, domain.PendingTxStatusFailed); err != nil {
	...
	if confirmations < p.RequiredConfirm {
		...
		_ = t.PendingTxs.UpdateConfirmations(ctx, p.TxHash, confirmations, domain.PendingTxStatusPending)
		return
	}

	// Validate against the order before confirming.
	order, err := t.Orders.GetByOrderID(ctx, p.OrderID)
	if err != nil {
	...
	if order.Status == domain.OrderStatusPaid || order.Status == domain.OrderStatusRefunded {
		// Already finalised.
		_ = t.PendingTxs.UpdateConfirmations(ctx, p.TxHash, confirmations, domain.PendingTxStatusConfirmed)
		return
	}
	if p.Amount == nil || p.Amount.Int64() != order.Amount {
		log.Warn("amount mismatch", "onchain", p.Amount, "order", order.Amount)
		_ = t.PendingTxs.UpdateConfirmations(ctx, p.TxHash, confirmations, domain.PendingTxStatusFailed)
		return
	}

	if err := t.PendingTxs.UpdateConfirmations(ctx, p.TxHash, confirmations, domain.PendingTxStatusConfirmed); err != nil {
		log.Warn("mark confirmed failed", "err", err)
		return
	}
	if err := t.Orders.UpdateStatus(ctx, order.OrderID, domain.OrderStatusPaid, ""); err != nil {
